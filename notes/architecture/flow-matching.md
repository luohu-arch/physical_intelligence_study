...
- 本地 PDF：`papers/architecture/Flow_Matching_for_Generative_Modeling_2210.02747.pdf`
...
| 消融因子 | 变化 | 结论 |
|---------|------|------|
| 概率路径 | OT vs VP vs VE | OT 路径训练最稳定，采样最快 |
| 推理步数 | 10 vs 5 vs 2 | 10 步达到高质量，5 步仍有竞争力 |
| Flow vs Diffusion loss | Flow vs DDPM | Flow 损失更稳定，无 SDE 采样方差 |

**核心结论**：OT 条件概率路径是 Flow Matching 的本质优势——使采样轨迹更直，将扩散的 100 步降至 10 步。
