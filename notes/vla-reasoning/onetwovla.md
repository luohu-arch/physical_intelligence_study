...
| 消融因子 | 变化 | 结论 |
|---------|------|------|
| Decision Token | with vs without | Decision Token 自适应切换是核心增益 |
| System 1+2 融合 | unified vs separate | 单模型融合优于分离式双系统 |
| 任务维度对比 | 长程/纠错/HRI/grounding | 四个维度均有提升，grounding 提升最大（88% vs 5%） |

**核心结论**：Decision Token 自适应推理使单模型在不同任务难度间无缝切换推理深度。
