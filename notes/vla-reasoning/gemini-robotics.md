...
| 消融因子 | 变化 | 结论 |
|---------|------|------|
| Gemini 2.0 vs 更小 VLM | 不同基座 | 更强 VLM 直接转化为更高操作成功率 |
| ER 推理模型分离 | ER vs 一体化 | 分离推理与动作有更好的可解释性 |
| Few-shot 微调样本数 | 100 vs 50 vs 20 | 约 100 示范足够微调新任务 |

**核心结论**：更大的 VLM = 更好的 VLA。分离推理（ER）和动作生成是可行的架构选择。缺少公开详细消融数据。
