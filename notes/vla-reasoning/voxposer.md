...
| 消融因子 | 变化 | 结论 |
|---------|------|------|
| LLM 代码质量 | GPT-4 vs GPT-3.5 | 更强 LLM 产生更精确的空间约束代码 |
| 3D Value Map 组合 | composed vs single | 可组合性使零样本复杂任务成为可能 |
| MPC vs 直接执行 | with vs without MPC | MPC 使轨迹更平滑且对感知噪声鲁棒 |

**核心结论**：LLM 代码 + 3D 可组合 value maps 是核心创新——将 LLM 常识通过代码转化为空间约束。
