...
- 本地 PDF：`papers/data-infra/Mobile_ALOHA_ACT_2401.02117.pdf`
...
| 消融因子 | 变化 | 结论 |
|---------|------|------|
| Action chunking | with vs without (single-step) | Chunking 是抑制复合误差的关键 |
| CVAE vs deterministic | CVAE vs 确定性预测 | CVAE 捕获示教中的多模态行为分布 |
| 数据量 | 不同示范数量 | 约 50 次示范可学会大多数任务 |

**核心结论**：Action chunking + CVAE 是 ACT 的精华——chunking 解决时序复合误差，CVAE 处理人类示教的多模态。
