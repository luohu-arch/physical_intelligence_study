...
- 本地 PDF：`papers/vla-architecture/OpenVLA_Open_Source_Vision_Language_Action_Model_2406.09246.pdf`
...
| 消融因子 | 变化 | 结论 |
|---------|------|------|
| 双视觉编码器 | DINOv2+SigLIP vs 单一 | 双编码器互补（DINOv2 空间 + SigLIP 语义） |
| LoRA rank | 64 vs 32 vs 16 | rank=64 最佳，平衡微调能力与过拟合 |
| 模型规模 | 7B vs 3B vs 1B | 7B 显著优于更小模型 |
| 预训练数据量 | 97万 vs 50万轨迹 | 数据量对泛化影响大，但边际递减 |
| 动作表示 | 连续回归 vs 离散 token | 连续回归在精度上优于离散 |

**核心结论**：双视觉编码器和足够大的 LoRA rank 是 OpenVLA 性能的关键。
