...
| 消融因子 | 变化 | 结论 |
|---------|------|------|
| LLM 层裁剪 | 50% vs 30% vs 0% | 裁剪 50% LLM 层几乎不损失性能 |
| Global-AdaLN | with vs without | Global-AdaLN 对动作特定条件关键 |
| 模型规模 vs 性能 | 950M vs 7B | 1/7 参数量匹敌更大模型 |

**核心结论**：LLM 层裁剪 50% + Global-AdaLN 是 FLOWER sub-billion 参数下匹敌大模型的核心。
