...
| 消融因子 | 变化 | 结论 |
|---------|------|------|
| 动作离散化 bin 数 | 256 vs 128 vs 64 | 256 bin 最优，过少损失精度 |
| ImageNet 预训练 | with vs without | 预训练显著提升小数据场景泛化 |
| FiLM 条件层数 | 深层 vs 浅层 | 深层 FiLM 对语言条件理解更关键 |
| 数据规模 | 不同任务数 | 多任务联合训练优于单任务，展示正迁移 |

**核心结论**：动作离散化和 ImageNet 预训练是 RT-1 成功最关键的两个设计选择，去掉任一个都导致显著性能下降。
