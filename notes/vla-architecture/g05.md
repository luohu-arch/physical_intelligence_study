...
| 消融因子 | 变化 | 结论 |
|---------|------|------|
| CoT 格式 | 8种格式 vs baseline (无 CoT) | CoT 显著提升长程任务和零样本泛化 |
| 子任务 CoT 权重 | 高权重 vs 均匀 | 子任务文本 CoT 对成功率贡献最大 |
| 活跃自由度自适应 | with vs without (固定维度) | 自适应 token 布局减少约 40% 无效 token |
| 视觉记忆帧数 | 6帧 vs 3帧 vs 1帧 | 6帧历史提供最优时空上下文 |

**核心结论**：Native CoT 和活跃自由度自适应是 G0.5 的两大效率支柱。待确认：部分消融数据来自技术报告和网页分析。
