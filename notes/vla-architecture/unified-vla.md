...
- Local PDF: `papers/vla-architecture/UniVLA_2506.19850.pdf`
...
| 消融因子 | 变化 | 结论 |
|---------|------|------|
| World Model post-training | with vs without | 世界模型后训练提升 10pp (LIBERO 95.5% vs 85.5%) |
| 离散 token 化策略 | 不同分词器 | 统一 token 序列是关键设计 |
| 训练阶段数 | 单阶段 vs 多阶段 | 多阶段训练（预训练-世界模型-动作）最优 |

**核心结论**：World model post-training 是 UniVLA 最大的增益来源——引入视频因果动态预测直接将 LIBERO 推至 95.5%。
