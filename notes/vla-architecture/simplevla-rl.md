...
| 消融因子 | 变化 | 结论 |
|---------|------|------|
| RL post-training | with vs without | RL 后训练显著提升（LIBERO SOTA） |
| Pushcut 现象 | 观察到的 emergent behavior | RL 训练中自发涌现出新策略 |
| 基座模型 | OpenVLA-OFT vs 其他 | 好的基座是 RL 后训练的前提 |

**核心结论**：RL 后训练可以超越模仿学习的性能上限；Pushcut 现象表明 RL 能诱导 emergent behavior。
