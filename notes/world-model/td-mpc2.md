...
| 消融因子 | 变化 | 结论 |
|---------|------|------|
| Decoder (像素重建) | with vs without | 无解码器训练更高效，性能不降甚至更优 |
| Policy prior | with vs without (纯随机采样) | Policy prior 显著减少 MPC 所需采样数 |
| Ensemble size | 2 vs 5 vs 10 Q functions | 2 个 Q ensemble 已足够 |
| 多任务 vs 单任务 | 80-task joint vs per-task | 多任务联合训练有正迁移 |
| TD vs Dreamer actor-critic | TD vs imagined rollouts | TD 更直接高效，不需要想象 |

**核心结论**：Decoder-free 设计——不做像素重建，用 TD-learning 直接提供任务信号，证明隐式世界模型在某些场景下更优。
